...
public class TraVarTUtils {
    ...
    public static Map<String, Feature> getFeatureMapFromRoot(final Feature feature) {
        ...
        final List<Feature> childFeatures = feature.getChildren().stream().flatMap(g -> g.getFeatures().stream()).collect(Collectors.toList());
        //add all children
    ...
    public static boolean isExcludes(final Constraint constraint) {
        ...
        formula = formula.cnf();
        final List<Literal> positiveLiterals = formula.literals().stream().filter(lit -> lit.phase())
    ...
    public static String deriveFeatureModelRoot(final Map<String, Feature> featureMap, final String rootName) {
        ...
        if (roots.size() > 1) {
            ...
            artificialRoot.getAttributes().put(HIDDEN_ATTRIBUTE, new Attribute<Boolean>("hidden", true));
