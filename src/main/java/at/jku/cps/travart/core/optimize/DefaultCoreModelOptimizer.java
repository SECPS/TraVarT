...
public class DefaultCoreModelOptimizer implements IModelOptimizer<FeatureModel> {
    ...
    private static void fixFalseOptionalFeaturesByFeatureGroupConstraints(final FeatureModel fm,
        ...
        for (final Feature childFeature : children) {
            final de.vill.model.constraint.Constraint requiredConstraint =
            ...
            if (TraVarTUtils.isRequires(constr)) {
                ...
                if (left != null && right != null && TraVarTUtils.isNegativeLiteral(left)
                    ...
                    final Feature rightFeature =
                        TraVarTUtils.getFeature(fm, ((LiteralConstraint) right).getLiteral());
    ...
    private static void fixFalseOptionalFeaturesByAbstractFeatureGroup(final FeatureModel fm,
                                                                       final Feature feature) {
        final Set<Feature> children = TraVarTUtils.getChildren(feature);
        for (final Feature child : children) {
        ...
        if (!children.isEmpty() && TraVarTUtils.isAbstract(feature)
            ...
            for (final Feature childFeature : children) {
                childMandatory =
                    childMandatory && TraVarTUtils.checkGroupType(childFeature, Group.GroupType.MANDATORY);
            }
            if (childMandatory) {
    ...
    private static void transformConstraintsToAlternativeGroup(final FeatureModel fm,
                                                               final Feature feature) {
        final Set<Feature> children = TraVarTUtils.getChildren(feature);
    ...
    private static void removeExcludesConstraint(final List<Constraint> ownConstraints,
                                                 final Constraint constraint) {
        final Optional<Constraint> constr =
            ownConstraints.stream().filter(oc -> oc.toString().equals(constraint.toString()))
                .findFirst();
        if (constr.isPresent()) {
    ...
    private static boolean toStringEquals(final List<Constraint> ownConstraints,
                                          final Constraint constraint) {
    ...
    private void fixDuplicatedConstraints(final FeatureModel fm) {
        final Iterator<de.vill.model.constraint.Constraint> iterator =
            TraVarTUtils.getOwnConstraints(fm).iterator();
        while (iterator.hasNext()) {
            final de.vill.model.constraint.Constraint constr = iterator.next();
            if (TraVarTUtils.getOwnConstraints(fm).stream()
                .filter(oc -> oc.toString().equals(constr.toString()))
                .count() > 1) {
                iterator.remove();
            }
        }
